Imports MySql.Data.MySqlClient
Imports System.Drawing.Printing
Imports System.IO
Public Class prodList
    Public Sub New()
        MyBase.New()
        InitializeComponent()
        Me.DoubleBuffered = True
    End Sub
    Public Sub RefreshDataGridView()
        connect()
        Me.DoubleBuffered = True
        ds = New DataSet
        da = New MySqlDataAdapter("Select product_id, product_name, product_type, product_supplier from tbl_products where active = 1", sqlconn)
        da.Fill(ds, "tbl_products")
        DataGridView1.DataSource = ds.Tables("tbl_products")
        DataGridView1.Columns(0).HeaderText = "ID"
        DataGridView1.Columns(1).HeaderText = "PRODUCT NAME"
        DataGridView1.Columns(2).HeaderText = "PRODUCT TYPE"
        DataGridView1.Columns(3).HeaderText = "SUPPLIER"
        disconnect_db()
    End Sub
    Public Sub RefreshDataGridView2()
        connect()
        Me.DoubleBuffered = True
        ds = New DataSet
        da = New MySqlDataAdapter("Select product_id, product_name, product_type, product_supplier from tbl_products where active = 0", sqlconn)
        da.Fill(ds, "tbl_products")
        DataGridView2.DataSource = ds.Tables("tbl_products")
        DataGridView2.Columns(0).HeaderText = "ID"
        DataGridView2.Columns(1).HeaderText = "PRODUCT NAME"
        DataGridView2.Columns(2).HeaderText = "PRODUCT TYPE"
        DataGridView2.Columns(3).HeaderText = "SUPPLIER"

        DataGridView2.Columns(0).Width = 100
        DataGridView2.RowTemplate.Height = 100

        DataGridView2.Columns("product_id").DefaultCellStyle.WrapMode = DataGridViewTriState.True
        DataGridView2.Columns("product_name").DefaultCellStyle.WrapMode = DataGridViewTriState.True
        DataGridView2.Columns("product_type").DefaultCellStyle.WrapMode = DataGridViewTriState.True
        DataGridView2.Columns("product_supplier").DefaultCellStyle.WrapMode = DataGridViewTriState.True

        disconnect_db()
    End Sub
    Private Sub prodList_Load_1(sender As Object, e As EventArgs) Handles MyBase.Load
        connect()
        RefreshDataGridView()
        btn_export2.Visible = False
        btn_print2.Visible = False
        btn_restore.Visible = False
        btn_delete.Hide()
        btn_back.Hide()
        txt_search2.Visible = False
        DataGridView2.Visible = False
        DataGridView1.Columns(0).Width = 100
        DataGridView1.RowTemplate.Height = 100
        DataGridView1.Columns("product_name").DefaultCellStyle.WrapMode = DataGridViewTriState.True
        DataGridView1.Columns("product_type").DefaultCellStyle.WrapMode = DataGridViewTriState.True
        DataGridView1.Columns("product_supplier").DefaultCellStyle.WrapMode = DataGridViewTriState.True
        Me.DoubleBuffered = True
        disconnect_db()
    End Sub
    Private Sub btn_update_Click(sender As Object, e As EventArgs) Handles btn_update.Click
        If updateProductForm.lbl_id.Text = "HSLLP-000" Then
            MessageBox.Show("No data selected!", "Error", MessageBoxButtons.OK, MessageBoxIcon.Error)
            Return
        End If
        updateProductForm.Show()
        updateSupplier.Close()
        userUpdateForm.Close()
    End Sub

    Private Sub DataGridView1_CellClick_1(sender As Object, e As DataGridViewCellEventArgs) Handles DataGridView1.CellClick
        lbl_id1.Text = DataGridView1.SelectedRows(0).Cells("product_id").Value.ToString
        updateProductForm.lbl_id.Text = DataGridView1.SelectedRows(0).Cells("product_id").Value.ToString
        updateProductForm.txt_pName.Text = DataGridView1.SelectedRows(0).Cells("product_name").Value.ToString
        updateProductForm.cmb_type.Text = DataGridView1.SelectedRows(0).Cells("product_type").Value.ToString
        updateProductForm.cmb_prodSupp.Text = DataGridView1.SelectedRows(0).Cells("product_supplier").Value.ToString

        lbl_id1.Text = DataGridView1.SelectedRows(0).Cells("product_id").Value.ToString
        stockInProduct.lbl_pID.Text = DataGridView1.SelectedRows(0).Cells("product_id").Value.ToString
        stockInProduct.lbl_pname.Text = DataGridView1.SelectedRows(0).Cells("product_name").Value.ToString
        stockInProduct.lbl_ptype.Text = DataGridView1.SelectedRows(0).Cells("product_type").Value.ToString
        stockInProduct.lbl_sname.Text = DataGridView1.SelectedRows(0).Cells("product_supplier").Value.ToString
    End Sub

    Private Sub btn_refresh_Click(sender As Object, e As EventArgs) Handles btn_refresh.Click
        connect()
        ds = New DataSet
        da = New MySqlDataAdapter("Select product_id, product_name, product_type, product_supplier from tbl_products", sqlconn)
        da.Fill(ds, "tbl_products")
        Me.DataGridView1.DataSource = ds.Tables("tbl_products")
        txt_search.ResetText()
        disconnect_db()
    End Sub

    Private Sub txt_search_TextChanged(sender As Object, e As EventArgs) Handles txt_search.TextChanged
        connect()

        ds = New DataSet
        da = New MySqlDataAdapter("Select product_id, product_name, product_type, product_supplier from tbl_products where product_name like @product_name order by product_name ASC", sqlconn)
        da.SelectCommand.Parameters.AddWithValue("@product_name", "%" & txt_search.Text & "%")
        da.Fill(ds, "tbl_products")
        DataGridView1.DataSource = ds.Tables("tbl_products")

        disconnect_db()
    End Sub

    Private Sub btn_print_Click(sender As Object, e As EventArgs) Handles btn_print.Click
        Dim printDialog As New PrintPreviewDialog()
        Dim printDocument As New PrintDocument()
        AddHandler printDocument.PrintPage, AddressOf PrintDocument1_PrintPage
        printDialog.Document = printDocument
        printDialog.ShowDialog()
    End Sub

    Private Sub PrintDocument1_PrintPage(sender As Object, e As Printing.PrintPageEventArgs) Handles PrintDocument1.PrintPage
        Dim bm As New Bitmap(Me.DataGridView1.Width, Me.DataGridView1.Height)
        DataGridView1.DrawToBitmap(bm, New Rectangle(5, 40, Me.DataGridView1.Width, Me.DataGridView1.Height))
        e.Graphics.DrawImage(bm, 0, 0)
    End Sub

    Private Sub btn_export_Click(sender As Object, e As EventArgs) Handles btn_export.Click
        ' Connect to the database
        connect()

        ' Create a SQL command to retrieve data from the database
        da = New MySqlDataAdapter("Select product_id, product_name, product_type, product_supplier FROM tbl_products where active = 1", sqlconn)
        Dim table As New DataTable()
        da.Fill(table)

        ' Create a new Excel file and add the DataGridView data to it
        Dim excel As New Microsoft.Office.Interop.Excel.Application()
        Dim workbook As Microsoft.Office.Interop.Excel.Workbook = excel.Workbooks.Add(Type.Missing)
        Dim worksheet As Microsoft.Office.Interop.Excel.Worksheet = Nothing
        worksheet = CType(workbook.Sheets("Sheet1"), Microsoft.Office.Interop.Excel.Worksheet)
        worksheet = workbook.ActiveSheet
        worksheet.Name = "Product List Data"
        For i As Integer = 1 To table.Columns.Count
            worksheet.Cells(1, i) = table.Columns(i - 1).ColumnName
        Next
        For i As Integer = 0 To table.Rows.Count - 1
            For j As Integer = 0 To table.Columns.Count - 1
                worksheet.Cells(i + 2, j + 1) = table.Rows(i)(j).ToString()
            Next
        Next

        ' Save the Excel file and close the Excel application
        Dim saveDialog As New SaveFileDialog()
        saveDialog.Filter = "Excel files (*.xlsx)|*.xlsx|All files (*.*)|*.*"
        saveDialog.FilterIndex = 2
        saveDialog.RestoreDirectory = True

        If File.Exists(saveDialog.FileName) Then
            If MessageBox.Show("The file already exists. Do you want to overwrite it?", "Confirm Overwrite", MessageBoxButtons.YesNo) = DialogResult.Yes Then
                File.Delete(saveDialog.FileName)
            Else
                Exit Sub ' Exit the sub if the user selects No or Cancel
            End If
        End If
        excel.Quit()

        ' Disconnect from the database
        disconnect_db()
    End Sub

    Private Sub btn_sIn_Click(sender As Object, e As EventArgs) Handles btn_sIn.Click
        If stockInProduct.lbl_pID.Text = "HSLLP-000" Then
            MessageBox.Show("No data selected!", "Error", MessageBoxButtons.OK, MessageBoxIcon.Error)
            Return
        End If
        stockInProduct.Show()
        updateProductForm.Close()
        updateSupplier.Close()
        userUpdateForm.Close()
    End Sub

    Private Sub btn_sDeactivate_Click(sender As Object, e As EventArgs) Handles btn_sDeactivate.Click
        DataGridView2.Visible = True
        RefreshDataGridView2()
        DataGridView1.Visible = False
        btn_back.Visible = True
        btn_sIn.Visible = False
        btn_restore.Visible = True
        btn_update.Visible = False
        btn_deactivate.Visible = False
        btn_delete.Visible = True
        txt_search2.Visible = True
        btn_export.Visible = False
        btn_export2.Visible = True
        btn_print2.Visible = True
        btn_print.Visible = False
        txt_search.ResetText()
        lbl_header.Text = "DEACTIVATED LIST"
    End Sub

    Private Sub btn_deactivate_Click(sender As Object, e As EventArgs) Handles btn_deactivate.Click
        If lbl_id1.Text = "HSLLP-000" Then
            MessageBox.Show("No data selected!", "Error", MessageBoxButtons.OK, MessageBoxIcon.Error)
            Return
        End If
        Dim confirm = MsgBox("ARE YOU SURE YOU WANT TO DEACTIVATE PRODUCT " & DataGridView1.CurrentRow.Cells("product_name").Value.ToString() & "?", MessageBoxButtons.YesNo + MsgBoxStyle.Information)
        If confirm = MsgBoxResult.Yes Then
            connect()
            ds = New DataSet
            da = New MySqlDataAdapter("update tbl_products set active = 0 where product_id = @product_id", sqlconn)
            da.SelectCommand.Parameters.AddWithValue("product_id", lbl_id1.Text)
            da.Fill(ds, "tbl_products")

            MsgBox("DEACTIVATION SUCCESSFUL")
            RefreshDataGridView()
            lbl_id1.Text = "HSLLP-000"

        Else

        End If
        lbl_id1.Text = "HSLLP-000"
        disconnect_db()
    End Sub

    Private Sub btn_back_Click(sender As Object, e As EventArgs) Handles btn_back.Click
        Me.DoubleBuffered = True
        DataGridView1.Visible = True
        RefreshDataGridView()
        DataGridView2.Visible = False
        btn_back.Visible = False
        btn_restore.Visible = False
        btn_sIn.Visible = True
        btn_delete.Visible = False
        btn_deactivate.Visible = True
        txt_search.Visible = True
        txt_search2.Visible = False
        btn_update.Visible = True
        btn_print2.Visible = False
        btn_export2.Visible = False
        btn_print.Visible = True
        btn_export.Visible = True
        lbl_header.Text = "PRODUCT LIST"
        lbl_id1.Text = "HSLLP-000"
        txt_search2.ResetText()
    End Sub

    Private Sub btn_restore_Click(sender As Object, e As EventArgs) Handles btn_restore.Click
        If lbl_id1.Text = "HSLLP-000" Then
            MessageBox.Show("No data selected!", "Error", MessageBoxButtons.OK, MessageBoxIcon.Error)
            Return
        End If

        Dim confirm = MsgBox("ARE YOU SURE YOU WANT TO ACTIVATE PRODUCT " & DataGridView2.CurrentRow.Cells("product_name").Value.ToString() & "?", MessageBoxButtons.YesNo + MsgBoxStyle.Information)
        If confirm = MsgBoxResult.Yes Then
            Try
                connect()
                Dim query = "UPDATE tbl_products SET Active = 1 WHERE product_id = @product_id"
                Using cmd As New MySqlCommand(query, sqlconn)
                    cmd.Parameters.AddWithValue("@product_id", lbl_id1.Text)
                    Dim rowsAffected = cmd.ExecuteNonQuery()
                    If rowsAffected > 0 Then
                        MsgBox("Product restored successfully.")
                        RefreshDataGridView2()

                        lbl_id1.Text = "HSLLP-000"

                    Else
                        MsgBox("Product not found.", MessageBoxIcon.Warning)
                    End If
                End Using
            Catch ex As Exception
                MsgBox("Error restoring product: " & ex.Message, MessageBoxIcon.Error)
            Finally
                disconnect_db()
            End Try

        End If
    End Sub

    Private Sub DataGridView2_CellClick(sender As Object, e As DataGridViewCellEventArgs) Handles DataGridView2.CellClick
        lbl_id1.Text = DataGridView2.SelectedRows(0).Cells("product_id").Value.ToString
    End Sub

    Private Sub btn_export2_Click(sender As Object, e As EventArgs) Handles btn_export2.Click
        ' Connect to the database
        connect()

        ' Create a SQL command to retrieve data from the database
        da = New MySqlDataAdapter("Select product_id, product_name, product_type, product_supplier FROM tbl_products where active = 0", sqlconn)
        Dim table As New DataTable()
        da.Fill(table)

        ' Create a new Excel file and add the DataGridView data to it
        Dim excel As New Microsoft.Office.Interop.Excel.Application()
        Dim workbook As Microsoft.Office.Interop.Excel.Workbook = excel.Workbooks.Add(Type.Missing)
        Dim worksheet As Microsoft.Office.Interop.Excel.Worksheet = Nothing
        worksheet = CType(workbook.Sheets("Sheet1"), Microsoft.Office.Interop.Excel.Worksheet)
        worksheet = workbook.ActiveSheet
        worksheet.Name = "Deactivated Product Data"
        For i As Integer = 1 To table.Columns.Count
            worksheet.Cells(1, i) = table.Columns(i - 1).ColumnName
        Next
        For i As Integer = 0 To table.Rows.Count - 1
            For j As Integer = 0 To table.Columns.Count - 1
                worksheet.Cells(i + 2, j + 1) = table.Rows(i)(j).ToString()
            Next
        Next

        ' Save the Excel file and close the Excel application
        Dim saveDialog As New SaveFileDialog()
        saveDialog.Filter = "Excel files (*.xlsx)|*.xlsx|All files (*.*)|*.*"
        saveDialog.FilterIndex = 2
        saveDialog.RestoreDirectory = True
        If File.Exists(saveDialog.FileName) Then
            If MessageBox.Show("The file already exists. Do you want to overwrite it?", "Confirm Overwrite", MessageBoxButtons.YesNo) = DialogResult.Yes Then
                File.Delete(saveDialog.FileName)
            Else
                Exit Sub ' Exit the sub if the user selects No or Cancel
            End If
        End If
        excel.Quit()

        ' Disconnect from the database
        disconnect_db()
    End Sub

    Private Sub btn_print2_Click(sender As Object, e As EventArgs) Handles btn_print2.Click
        Dim printDialog As New PrintPreviewDialog()
        Dim printDocument As New PrintDocument()
        AddHandler printDocument.PrintPage, AddressOf PrintDocument2_PrintPage
        printDialog.Document = printDocument
        printDialog.ShowDialog()
    End Sub

    Private Sub PrintDocument2_PrintPage(sender As Object, e As PrintPageEventArgs) Handles PrintDocument2.PrintPage
        Dim bitmap As New Bitmap(Me.DataGridView2.Width, Me.DataGridView2.Height)
        Me.DataGridView2.DrawToBitmap(bitmap, Me.DataGridView2.DisplayRectangle)

        ' Add header with logo and text
        Dim headerFont As New Font("Arial", 30, FontStyle.Bold)
        Dim headerText As String = "STOCKSLOGIX"
        Dim headerHeight As Integer = 100
        Dim headerRect As New Rectangle(0, 0, e.PageBounds.Width, headerHeight)
        Dim logoRect As New Rectangle(10, 10, 80, 80)
        e.Graphics.FillRectangle(Brushes.LightGray, headerRect)

        e.Graphics.DrawString(headerText, headerFont, Brushes.Black, logoRect.Right + 10, 15)
        Dim headerTextSize As SizeF = e.Graphics.MeasureString(headerText, headerFont)
        Dim headerTextRect As New RectangleF((e.PageBounds.Width - headerTextSize.Width) / 2, logoRect.Bottom + 10, headerTextSize.Width, headerTextSize.Height)
        e.Graphics.DrawString(headerText, headerFont, Brushes.SteelBlue, headerTextRect)

        ' Calculate the bounds for the DataGridView
        Dim dgvWidth As Integer = e.PageBounds.Width
        Dim dgvHeight As Integer = e.PageBounds.Height - headerRect.Height
        Dim dgvRect As New Rectangle(0, headerRect.Bottom, dgvWidth, dgvHeight)

        ' Adjust the size of the DataGridView to fit the calculated bounds
        Dim originalSize As Size = Me.DataGridView2.Size
        Me.DataGridView2.Size = dgvRect.Size

        ' Print the DataGridView
        e.Graphics.DrawImage(bitmap, dgvRect)

        ' Restore the size of the DataGridView to its original size
        Me.DataGridView2.Size = originalSize
    End Sub

    Private Sub btn_delete_Click(sender As Object, e As EventArgs) Handles btn_delete.Click

        If lbl_id1.Text = "HSLLP-000" Then
            MessageBox.Show("No data selected!", "Error", MessageBoxButtons.OK, MessageBoxIcon.Error)
            Return
        End If
        Dim confirm = MsgBox("ARE YOU SURE YOU WANT TO PERMANENTLY DELETE PRODUCT " & DataGridView2.CurrentRow.Cells("product_name").Value.ToString() & "?", MessageBoxButtons.YesNo + MsgBoxStyle.Exclamation)
        If confirm = MsgBoxResult.Yes Then
            Try
                connect()
                Dim query = "DELETE FROM tbl_products WHERE product_id = @product_id"
                Using cmd As New MySqlCommand(query, sqlconn)
                    cmd.Parameters.AddWithValue("@product_id", lbl_id1.Text)
                    Dim rowsAffected = cmd.ExecuteNonQuery()
                    If rowsAffected > 0 Then
                        MsgBox("Product Deleted Successfully.")
                        RefreshDataGridView2()

                        lbl_id1.Text = "HSLLP-000"

                    Else
                        MsgBox("Product not found.", MessageBoxIcon.Warning)
                    End If
                End Using
            Catch ex As Exception
                MsgBox("Error restoring product: " & ex.Message, MessageBoxIcon.Error)
            Finally
                disconnect_db()
            End Try
        End If
    End Sub

    Private Sub txt_search2_TextChanged(sender As Object, e As EventArgs) Handles txt_search2.TextChanged
        connect()

        ds = New DataSet
        da = New MySqlDataAdapter("Select product_id, product_name, product_type, product_supplier from tbl_products where active = 0 AND product_name like @product_name order by product_name ASC", sqlconn)
        da.SelectCommand.Parameters.AddWithValue("@product_name", "%" & txt_search2.Text & "%")
        da.Fill(ds, "tbl_products")
        DataGridView2.DataSource = ds.Tables("tbl_products")

        disconnect_db()
    End Sub
End Class